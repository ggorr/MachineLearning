...
class MidSet2D(NodeSet, metaclass=ABCMeta):
	...
	def prePropBoth(self, trSiz: int, teSiz: int):
		self.trX = np.empty((self.xchs, self.xdim, trSiz)) if isinstance(self.xdim, int) else \
			np.empty((self.xchs, self.xdim[0], self.xdim[1], trSiz))
		self.trY = np.empty((self.ychs, self.ydim, trSiz)) if isinstance(self.xdim, int) else \
			np.empty((self.ychs, self.ydim[0], self.ydim[1], trSiz))
		self.gradX = np.empty_like(self.trX)
		self.gradY = np.empty_like(self.trY)
		self.teX = np.empty((self.xchs, self.xdim, teSiz)) if isinstance(self.xdim, int) else \
			np.empty((self.xchs, self.xdim[0], self.xdim[1], teSiz))
		self.teY = np.empty((self.ychs, self.ydim, teSiz)) if isinstance(self.xdim, int) else \
			np.empty((self.ychs, self.ydim[0], self.ydim[1], teSiz))
	...
	def prePropPr(self, prSiz: int):
		self.prX = np.empty((self.xchs, self.xdim, prSiz)) if isinstance(self.xdim, int) else \
			np.empty((self.xchs, self.xdim[0], self.xdim[1], prSiz))
		self.prY = np.empty((self.ychs, self.ydim, prSiz)) if isinstance(self.xdim, int) else \
			np.empty((self.ychs, self.ydim[0], self.ydim[1], prSiz))
