...
class MaxPool2D(MidSet2D):
	...
	def prePropBoth(self, trSiz: int, teSiz: int):
		...
		self.gradX = np.empty_like(self.trX)
		self.gradY = np.empty((self.ychs, self.ydim[0], self.ydim[1], trSiz))
		self.teX = np.empty((self.xchs, self.xdim[0], self.xdim[1], teSiz))
		self.teY = np.empty((self.ychs, self.ydim[0], self.ydim[1], teSiz))

		# self.trMed = np.empty((self.xchs, self.ydim[0], self.xdim[1], trSiz))
		self.teMed = np.empty((self.xchs, self.ydim[0], self.xdim[1], teSiz))
	...
	def prePropPr(self, prSiz: int):
		...
		self.prMed = np.empty((self.xchs, self.ydim[0], self.xdim[1], prSiz))
