...
def channelFirst(*x: Union[np.ndarray, list, tuple]) -> Union[np.ndarray, tuple]:
	if len(x) == 1:
		return x[0].T if x[0].ndim == 3 else x[0].transpose((3, 1, 2, 0))
	return tuple(o.T if o.ndim == 3 else o.transpose((3, 1, 2, 0)) for o in x)


def channelLast(*x: Union[np.ndarray, list, tuple]) -> Union[np.ndarray, tuple]:
	if len(x) == 1:
		return x[0].T if x[0].ndim == 3 else x[0].transpose((3, 1, 2, 0))
	return tuple(o.T if o.ndim == 3 else o.transpose((3, 1, 2, 0)) for o in x)
...
def addChannel(*x: Union[np.ndarray, list, tuple]) -> Union[np.ndarray, tuple]:
	if len(x) == 1:
		return channelFirst(x[0][..., np.newaxis])
	return tuple(channelFirst(o[..., np.newaxis]) for o in x)
