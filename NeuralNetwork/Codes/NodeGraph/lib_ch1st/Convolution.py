...
def partialProp1D(x, y, b, w, stride, filSiz, start, siz):
	s = 0
	for k in range(y.shape[1]):
		y[:, k, start:start + siz] = np.tensordot(w, x[:, s:s + filSiz, start:start + siz])
		s += stride
	y[:, :, start:start + siz] += b
...
class Conv1D(FitSet1D):
	...
	def preFit(self, **kwargs):
		...
		if self.initB is None:
			self.initB = np.zeros((self.ychs, 1, 1))
		elif self.initB.ndim == 1:
			self.initB = self.initB[:, np.newaxis, np.newaxis]
		if self.initW is None:
			self.initW = np.random.rand(self.ychs, self.xchs, self.filSiz)
		self.B = self.initB.copy() if self.biased else np.zeros((self.ychs, 1, 1))
	...
	def prePropPr(self, prSiz: int):
		self.xNeeds = len(self.prevSets)
		self.prX = np.zeros((self.xchs, self.xdimExt, prSiz))
...
class Conv2D(FitSet2D):
	...
	def preFit(self, **kwargs):
		...
		self.mom = kwargs['momentum']
		if self.initB is None:
			self.initB = np.zeros((self.ychs, 1, 1, 1))
		elif self.initB.ndim == 1:
			self.initB = self.initB[:, np.newaxis, np.newaxis, np.newaxis]
		if self.initW is None:
			self.initW = np.random.rand(self.ychs, self.xchs, self.filSiz[0], self.filSiz[1])
		self.B = self.initB.copy() if self.biased else np.zeros((self.ychs, 1, 1, 1))
		self.W = self.initW.copy()
		self.BPrev = np.zeros_like(self.B)
		self.WPrev = np.zeros_like(self.W)
		self.gradB = np.zeros_like(self.B)
	...
	def prePropBoth(self, trSiz: int, teSiz: int):
		self.xNeeds = len(self.prevSets)
		self.trX = np.zeros((self.xchs, self.xdimExt[0], self.xdimExt[1], trSiz))
		self.trY = np.empty((self.ychs, self.ydim[0], self.ydim[1], trSiz))
		self.gradX = np.empty_like(self.trX)
		self.gradY = np.empty_like(self.trY)
		self.teX = np.zeros((self.xchs, self.xdimExt[0], self.xdimExt[1], teSiz))
	...
	def prePropPr(self, prSiz: int):
		self.xNeeds = len(self.prevSets)
		self.prX = np.zeros((self.xchs, self.xdimExt[0], self.xdimExt[1], prSiz))
