...
def contour(ng: NodeGraph, xr: Union[List, Tuple], yr: Union[List, Tuple], title: str = None):
	"""
	...
	x = np.arange(xr[0], xr[1], delta).astype(np.float32)
	y = np.arange(yr[0], yr[1], delta).astype(np.float32)
	X, Y = np.meshgrid(x, y)
	Z = np.zeros_like(X)
