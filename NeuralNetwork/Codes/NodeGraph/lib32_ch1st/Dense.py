...
class Dense1D(FitSet1D):
	...
	def preFit(self, **kwargs):
		if self.initB is None:
			self.initB = np.zeros((self.xchs, self.ydim, 1), np.float32)
		if self.initW is None:
			self.initW = (np.random.random((self.xchs, self.ydim, self.xdim)).astype(np.float32) * 2 - 1) / ( self.xdim * self.ydim * self.ychs)
		self.B = self.initB.astype(np.float32) if self.biased else np.zeros((self.xchs, self.ydim, 1), np.float32)
	...
	def updateAdaptive(self, lr: np.float32):
		"""
		:param lr: learning rate
		"""
