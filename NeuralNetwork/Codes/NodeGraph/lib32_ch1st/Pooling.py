...
class MaxPool2D(MidSet2D):
	...
	def prePropBoth(self, trSiz: int, teSiz: int):
		self.trX = np.empty((self.xchs, self.xdim[0], self.xdim[1], trSiz), np.float32)
		# self.trY = np.empty((self.ychs, self.ydim[0], self.ydim[1], trSiz), np.float32)
		self.gradX = np.empty_like(self.trX)
		self.gradY = np.empty((self.ychs, self.ydim[0], self.ydim[1], trSiz), np.float32)
		self.teX = np.empty((self.xchs, self.xdim[0], self.xdim[1], teSiz), np.float32)
		self.teY = np.empty((self.ychs, self.ydim[0], self.ydim[1], teSiz), np.float32)

		# self.trMed = np.empty((self.xchs, self.ydim[0], self.xdim[1], trSiz), np.float32)
		self.teMed = np.empty((self.xchs, self.ydim[0], self.xdim[1], teSiz), np.float32)
		self.ind0 = np.empty((self.xchs, self.ydim[0], self.xdim[1], trSiz), int)
		self.ind1 = np.empty((self.ychs, self.ydim[0], self.ydim[1], trSiz), int)
	...
	def prePropPr(self, prSiz: int):
		self.prX = np.empty((self.xchs, self.xdim[0], self.xdim[1], prSiz), np.float32)
		self.prY = np.empty((self.ychs, self.ydim[0], self.ydim[1], prSiz), np.float32)
		self.prMed = np.empty((self.xchs, self.ydim[0], self.xdim[1], prSiz), np.float32)
