...
class Softplus1D(Activation1D):
	...
	def pushTrX(self):
		np.logaddexp(F0, self.trX, self.trY)

	def pushTeX(self):
		np.logaddexp(F0, self.teX, self.teY)

	def pushPrX(self):
		np.logaddexp(F0, self.prX, self.prY)
	...
	def pullGradY(self):
		...
		"""
		np.multiply(F1 - np.exp(-self.trY), self.gradY, self.gradX)
