...
class Dense1D(FitSet1D):
	...
	def preFit(self, **kwargs):
		if self.initB is None:
			self.initB = np.zeros((self.ydim, self.ychs))
		if self.initW is None:
			self.initW = (np.random.rand(self.xdim, self.ydim, self.ychs) * 2 - 1) / (self.xdim * self.ydim * self.ychs)
		self.B = self.initB.copy() if self.biased else np.zeros((self.ydim, self.ychs))
