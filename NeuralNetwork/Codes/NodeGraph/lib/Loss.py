...
class LossFunc1D(NodeSet, metaclass=ABCMeta):
	...
	def trAccuracy(self) -> float:
		if self.trT.shape[1] == 1:
			return np.sum((self.trY > 0.5) == (self.trT > 0.5)) / self.trT.shape[0]
		else:
	...
	def teAccuracy(self) -> float:
		if self.teT.shape[1] == 1:
			return np.sum((self.teY > 0.5) == (self.teT > 0.5)) / self.teT.shape[0]
		else:
...
class Bce1D(LossFunc1D):
	...
	def baseVec(self, t: np.ndarray) -> np.ndarray:
		return -np.sum(t * np.log(t + LOSS_E0) + (1 - t) * np.log((1 - t) + LOSS_E0), (1, 2))

	def lossVec(self, t: np.ndarray, y: np.ndarray) -> np.ndarray:
		return -np.sum(t * np.log(y + LOSS_E0) + (1 - t) * np.log((1 - y) + LOSS_E0), (1, 2))
...
class OneHotCce1D(LossFunc1D):
	...
	def baseVec(self, t: np.ndarray) -> np.ndarray:
		return np.zeros(t.shape[0], float)
